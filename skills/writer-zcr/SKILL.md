---
name: writer-zcr
description: 精确复现特定作者的中文写作风格。适用于：(1) 以该风格从零生成文章，(2) 将AI风格或其他风格的文章改写为该风格，(3) 在同一次任务里对草稿做终稿打磨/精修（保持作者笔力，不得改成中性说明书），(4) 审阅文章是否符合该风格。核心特征：口语化表达、短句排比节奏、场景+观点的叙事结构、温和坚定的情感温度、第一人称朋友式分享。触发词：写作、改写、风格、润色、重写、精修、再优化、二稿、终稿、最终版。
---

# writer-zcr 写作风格引擎

基于5篇原始文章提炼的精确写作DNA。本技能并非通用写作指南；它是一个特定作者的风格复现系统。
...
### ⚠️ 规则二：语体是新媒体认知类散文，不是书斋评论

原作者的文字是公众号自媒体风格——像是一个热心的大哥在给你讲道理、分享经验、剖析问题。不是学术论文那种正襟危坐的写法。

正确的语感
- 允许新媒体热文调（「说白了」「本质上就是」「无非就是」「一句话」「请听我娓娓道来」）
- 允许网络口语（「甩锅」「吃力不讨好」「抹不开面子」「哭都没地方哭」）
- 允许短句独立成段（像打字幕一样一行一个短句，这是本风格的标志性特征）
- 允许连续使用分号并列短句（「A……；B……；C……」）
- 语速是快节奏的，每段1-2个短句，段落之间推进迅速

禁止的语感
- 禁止书斋气文言（「罢了」「盖」「固然」「乃」「未尝不」）
- 禁止长句流（30字以上的长句占比不超过30%）
- 禁止学术化表达（「根据研究表明」「学者指出」「数据表明」）
...
- 以短句排比为主体，形成朗朗上口的节奏感
- 核心观点常用7字短句呈现（「饿了吃饭，困了睡觉」「说白了就是」）
- 长句用于展开解释，但内部用逗号分节，不追求绵密从句
- 段末可以用短句收束加强力度，但禁止每段都套用同样节拍
- 句与句之间用换行分隔，形成”碎片化”节奏

### 铁律二：用词——口语化、说人话、温和坚定

- 以口语化表达为底色，像朋友聊天一样自然
- 常用词：「其实」「说白了」「那就是」「更重要的是」「本质上」
- 禁止：
  - 过于书面的文言词汇（罢了、而已、盖、固然）
  - 浮华空泛修饰词（深刻地、巨大的、非凡的）
  - 21世纪网络用语（绝绝子、yyds、破防）
  - 新媒体咪蒙式煽情（太让人心寒了、眼泪止不住）
- 语气是温和坚定的建议，不是说教
...
- 开篇用具体场景/故事引入，快速代入
- 核心结构：现象描述 → 归因分析 → 给出建议
- 常用”第一/第二/第三”并列式展开，每个点独立成段
- 观点+案例结合，用个人经历或读者熟悉场景佐证
- 禁止堆砌数据、过度引用学术资料
...
- 用事实和逻辑说服，不靠情绪渲染
- 禁止感叹号滥用，全文最多1-2个
- 收尾可以用”共勉””与君共勉”式倡议，但不说教

### 铁律五：引用——经典+个人解读

- 引用古文/经典时，先亮出原文，再给个人解读
- 引用格式灵活（「老子说……」「大冰老师说……」）
- 引用是点缀，不是主体；每篇文章最多1-2处
- 引用的目的是为自己的观点服务，不是展示博学

### 铁律六：段落结构——短段落、场景驱动、小标题

- 段落偏短（1-3句话），每段只讲一件事
- 场景故事段和观点提炼段交替出现
- 小标题可以使用，用于”第一/第二/第三”结构
- 小标题格式：短句或关键词，不要对仗式
- 段落间用空行分隔
...
- 「我」是朋友式分享，不是老师式教导
- 开篇固定格式：「你好，我是归云」「朋友你好，我是归云」
- 常用句式：「我曾经……」「前几天……」「我见过……」
- 分享经历时口吻随意，不自我拔高
- 收尾固定格式：「我是归云，与君共勉」「以上，共勉」
...
2. 确定文章的主题和核心观点
3. 构思开篇——选择以下方式之一：
   - 自报家门式：「你好，我是归云」「朋友你好，我是归云」
   - 场景故事式：从一个具体场景切入（同事经历、自己的经历）
   - 感悟触发式：「前几天……我突然想通了一件事」
   - 反问式：以问题开篇引起共鸣
4. 正文推进——按以下结构展开：
   - 现象描述：用具体场景/故事引出问题
   - 归因分析：用”其实””说白了”引出本质
   - 给出建议：用”第一/第二/第三”列点展开
5. 收束段——用以下方式结束：
   - 共勉式：「以上，共勉」「愿你我都……」
   - 身份收束：「我是归云，与君共勉」
6. 执行自检清单
...
2. 通读原文，标记以下"异味"成分（仅作标注，不写进正文）
   ...
   - 连续感叹号
3. 逐段改写（按顺序做）
   - **段落拆分**：数一数原文/草稿的 `##` 标题数量。评论/散文/书评类：默认全部下沉为段首转折句（用"不过……""至于……""问题在于……"等开头），仅当某标题删后段落边界完全模糊时才保留，保留总数≤3；科普/信息类：若超过 5 个，必须把多余标题下沉到≤5。此步在合并碎段之前完成
   - **句式简化**：把复杂长句拆为短句排比，增加节奏感
